...
public class SerialInputController : MonoBehaviour {
	...
	void updateData(bool[] newData){
		...
		//18input
		for (int i=0; i<18; i++) {
			if(newData[i]!=isToushed[i]){
				touchBuffer[i]++;
				if(touchBuffer[i]>=buffersize){
					touchBuffer[i]=0;
					isToushed[i]=newData[i];
				}
			}else{
				touchBuffer[i]=0;			}
		}
	...
	void FixedUpdate () {
		if (!sp.IsOpen) {
			...
			try {
				...
				if(words.Length==4)touchController(words);//the last word is return
